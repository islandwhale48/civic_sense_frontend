...
export default function Profile({ issues, onToggleUpvote }) {
  ...
  // Filter reported issues (or user's own reports)
  const myReportedIssues = issues.filter(
    (i) => i.reporter.name === userProfile.name || i.reporter.name === 'Rahul Sharma'
  );
  const myUpvotedIssues = issues.filter((i) => i.upvotedByUser);
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-800 bg-gradient-to-r from-slate-900 via-slate-900/90 to-blue-950/30 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 relative z-10">
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <p className="text-xs text-slate-400 mb-4">{userProfile.email} • Member since {userProfile.joined}</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl text-center">
                <p className="text-xl font-extrabold text-emerald-400">
                  {myReportedIssues.filter(i => i.status === 'resolved').length}
                </p>
                <p className="text-[11px] text-slate-400">Resolved</p>
              </div>
              <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl text-center">
                <p className="text-xl font-extrabold text-amber-400">{userProfile.stats.upvotesGiven}</p>
                <p className="text-[11px] text-slate-400">Upvotes Given</p>
              </div>
              <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl text-center">
                <p className="text-xl font-extrabold text-purple-400">{userProfile.stats.civicKarma}</p>
